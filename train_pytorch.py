"""
train_pytorch.py — Feedforward Neural Network for Emotional Load Regression
============================================================================
Architecture:
  Input(15) → FC(64) → BN → ReLU → Dropout(0.3)
             → FC(32) → BN → ReLU → Dropout(0.2)
             → FC(16) → ReLU
             → FC(1)  → Sigmoid  →  output ∈ [0, 1]

Training:
  - Loss       : MSE
  - Optimiser  : AdamW (weight decay 1e-4)
  - Scheduler  : CosineAnnealingLR
  - Early stop : patience 15 epochs on val loss

Run:
  python train_pytorch.py
Outputs:
  nn_model.pth    — saved model state dict
  plots/          — loss curve, prediction scatter
"""
...
class EmotionalLoadNet(nn.Module):
    """
    Feedforward network for emotional load regression.
    Takes 15 scaled linguistic features, outputs a single float in [0, 1].

    Architecture chosen for the feature count (15 inputs):
      - Two hidden layers with batch normalisation for stable training
      - Dropout for regularisation on a small dataset
